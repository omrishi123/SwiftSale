...
export default function NewSaleModal({ isOpen, onClose, openModal, changeView }: NewSaleModalProps) {
  ...
  }, [isOpen, appData.settings.defaultTax]);

  const availableProducts = useMemo(() => appData.stock.filter(p => p.stock > 0), [appData.stock]);
  ...
  const handleAddItem = () => {
    ...
    if (totalQty > product.stock) {
      toast({ variant: 'destructive', title: "Not enough stock", description: `Only ${product.stock} units available.`});
      return;
    }

    if (existingItem) {
      setSaleItems(saleItems.map(i => i.sku === product.sku ? { ...i, quantity: totalQty } : i));
    } else {
  ...
  const financialSummary = useMemo(() => {
    ...
    const finalAmountPaid = typeof amountPaid === 'number' ? amountPaid : grandTotal;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNewCustomer) {
      if (!newCustomerName) {
        toast({ variant: 'destructive', title: 'Customer name is required for new customers.' });
        return;
      }
      customer = addCustomer({ name: newCustomerName, phone: newCustomerPhone, address: newCustomerAddress });
    } else {
      if (!selectedCustomerId) {
        toast({ variant: 'destructive', title: 'Please select an existing customer.' });
        return;
      }
      customer = appData.customers.find(c => c.id === parseInt(selectedCustomerId));
    }
    if (!customer) {
        toast({ variant: 'destructive', title: 'Could not find or create customer.' });
        return;
    }
    ...
    const newSale = addSale({
      ...
      amountPaid: typeof amountPaid === 'number' ? amountPaid : financialSummary.grandTotal,
